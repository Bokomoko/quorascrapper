...
class _ServiceFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:  # always include
...
def _build_json_formatter(indent: int | None = None) -> logging.Formatter:
    if jsonlogger is None:
        # Fallback to simple formatter emitting JSON-ish string
...
def init_logging(service: str, default_level: str = "INFO") -> logging.Logger:
    ...
    if not _CONFIGURED:
        # Clear any default handlers set by libraries
