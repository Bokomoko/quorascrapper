...
| Date | Change |
|------|--------|
| 2026-06-13 | **Extension v0.9.30 — owner-scoped SAVED (0 for fresh profiles) + GraphQL-sourced TOTAL + Max prefill.** Fixes the popup stats block showing the WRONG **SAVED** and a blank **TOTAL** on a never-ingested profile's `/answers` page. *SAVED was 16,657 (wrong):* the periodic saved poll (`fetchKnownCount()`) ran with an unresolved `activeProfileUrl` (null on open, before `refreshTargetTab` resolved the owner), so `knownUrl()` omitted `profile_url` and serve fell back to the global legacy **`answers`** collection (~16,657 from the initial backfill) — a different profile's/global count, not THIS owner's. *Fix:* `fetchKnownCount()` now returns `null` (sends NO request) until `activeProfileUrl` is resolved, so SAVED is ALWAYS scoped to the current owner's own `profile_<userid>` collection; a never-ingested profile correctly reads **0** (its own empty collection), never the global count. `fetchProfileTotal()` also pins `activeProfileUrl` to the owner immediately and re-polls SAVED once scoped. *TOTAL was "—" (wrong):* DOM/og:description extraction (`extractProfileStats`) failed for the profile. *Fix:* per the user's direction, TOTAL now comes from the **GraphQL/relay data Quora embeds in the page**, not DOM scraping. The owner's total = `numPublicAnswers` on the owner `User` record, which sits immediately before the answers-tab connection (`recentPublicAndPinnedAnswersConnection`) in the embedded relay JSON (verified against the captured HAR: owner `numPublicAnswers:17441` directly precedes the connection; other users' records are followed by `network`/`postsCount`). New `scrape.js` `extractOwnerInfo()` (exposed via a `getProfileInfo` message) returns `{profile_url (canonical /profile/<slug> from location.href), profile_name, profile_display_name, answer_count, uid}` — GraphQL first, `extractProfileStats` DOM only as a last resort. *Owner keying:* on `/profile/<slug>/answers` the owner = `<slug>`; canonical = `https://<host>/profile/<slug>`, derived identically in the content script and popup (`profileUrlFromAnswers`) so SAVED scoping matches the ingest-time `profile_url`. The popup reads owner info on open + profile change (`readOwnerInfoFromTab`), sets TOTAL = `answer_count`, scopes SAVED to the owner, and resets the per-profile guards on profile change. *Max prefill:* the corrected GraphQL owner total now also one-shot prefills the **Max new answers** input (`prefillMaxFromTotal()`, reset per profile via `maxAutoFilled`, never clobbering a mid-session/user-typed value) so TOTAL and Max show the same real number. Scrape `/answers`-gating, per-profile reset, and streaming publish are unchanged. *Files:* `extension/scrape.js`, `extension/popup.js`, `extension/manifest.json` (v0.9.30). **Reload reminder:** reload the extension at `chrome://extensions` and refresh the open Quora `/answers` tab so the new content script + popup load. |
| 2026-06-13 | **Extension v0.9.29 — fresh per-profile total on open + Scrape button gated to /answers pages.** *Issue 1 (stale "Total on Quora"):* the embedded panel iframe survives tab navigation, so the module-level `profileState.total`/`.saved` and the `maxAutoFilled` prefill guard kept showing (and auto-prefilling Max with) a PREVIOUS/other profile's count when the popup opened on a different profile's `/answers` tab. *Fix:* `fetchProfileTotal()` now **resets** `profileState.total`/`.saved` + `maxAutoFilled` the moment the resolved `profileUrl` differs from `activeProfileUrl`, and **prefers freshness on open** — it reads the CURRENT active tab's DOM (`readStatsFromTab`) FIRST and only falls back to the per-profile-scoped cache (`profileStatsCache[profileUrl]`) / background-tab fetch, so a stale TTL-window count never wins over the live page. A new `refreshTargetTab()` re-resolves the active tab and re-loads the total whenever the resolved profile changes; `chrome.tabs.onActivated`/`onUpdated` listeners (debounced) drive re-evaluation while the panel is open. *Issue 2 (Scrape button always enabled):* `#start` was always clickable and only validated on click. *Fix:* new `updateScrapeGate(tab)` disables/greys `#start` (with a tooltip + status hint explaining why) unless the active target tab is a Quora profile `/answers` page (`isAnswersPage`), re-evaluated on popup open, on every active/updated-tab change, and in the run's `finally` block (the re-enable now respects the gate instead of unconditionally `disabled = false`). *Files:* `extension/popup.js`, `extension/manifest.json` (v0.9.29). |
